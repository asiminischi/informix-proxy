...
This runs the protobuf compiler to generate Java classes from `src/main/proto/informix.proto`, compiles the proxy server, and produces a fat JAR at `target/informix-grpc-proxy-1.1.0.jar`.
...
target/
  ...
  informix-grpc-proxy-1.1.0.jar    -- fat JAR after build
```
...
java -jar target/informix-grpc-proxy-1.1.0.jar
```
